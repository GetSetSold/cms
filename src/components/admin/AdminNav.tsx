...
export function AdminNav({ role, newLeads }: { role: Role; newLeads: number }) {
      ...
      {ITEMS.filter((i) => i.roles.includes(role)).map((i) => {
        ...
        return (
          <Link key={i.href} href={i.href} aria-current={active ? "page" : undefined}
            className={`flex h-10 items-center gap-2.5 rounded-lg px-3 ${active ? "bg-[#2A2D35] text-white" : "hover:bg-[#1F2229]"}`}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={i.icon} /></svg>
            {i.label}
