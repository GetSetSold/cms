...
import type { CmsForm, Page, Section, SvgAsset } from "@/lib/types";

export default async function EditPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { supabase } = await requireStaff(["admin", "editor"]);
  const [{ data: page }, { data: sections }, { data: blockTypes }, { data: svgs }, { data: forms }] = await Promise.all([
  ...
  return (
    <PageBuilder page={page as Page} sections={(sections ?? []) as Section[]}
      blockTypes={blockTypes ?? []} svgs={(svgs ?? []) as SvgAsset[]} forms={(forms ?? []) as CmsForm[]} />
  );
}
