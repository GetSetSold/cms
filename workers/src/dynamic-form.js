...
function renderQuestion(q, nameOverride, qidOverride, sectionColumns = 2) {
  if (q.type === "section_header") {
    return `<div class="df-field df-header-field" style="grid-column:span ${sectionColumns};">
  ...
  if (q.type === "multi_line") {
    ...
    inputHtml = `<div class="df-choice-group">${opts.map((o) => `<label class="df-radio"><input type="radio" name="${escAttr(name)}" value="${escAttr(o.value)}" data-qid="${qid}" ${required}> ${esc(o.label)}</label>`).join("")}</div>`;
  } else if (q.type === "multicheck") {
    inputHtml = `<div class="df-choice-group">${options.map((o) => `<label class="df-checkbox"><input type="checkbox" name="${escAttr(name)}[]" value="${escAttr(o.value)}" data-qid="${qid}"> ${esc(o.label)}</label>`).join("")}</div>`;
  ...
  return `<div class="df-field" data-question-id="${qid}" ${styleAttr}>
...
function renderRepeater(q, children, sectionColumns = 2) {
  ...
  return `<div class="df-field df-repeater" style="grid-column:span ${sectionColumns};" data-repeater-id="${q.id}" data-repeater-source="${escAttr(sourceId)}" data-repeater-min="${min}" data-repeater-max="${max}">
...
export function renderDynamicForm(props = {}, form) {
  ...
  return `<section class="block df-block">
    <div class="df-wrap">
